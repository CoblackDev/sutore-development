<?php
/**
 * Plugin Name: WooCommerce Sözleşmeleri
 * Plugin URI: https://eguler.net
 * Description: Pazaryeri için sözleşmeler eklentisi.
 * Author: EMRE GÜLER
 * Author URI: https://eguler.net
 * Version: 2.0
 * Text Domain: woocontracts
 *
 * Copyright: (c) 2020 EMRE GÜLER (iletisim@eguler.net)
 *
 * @author    EMRE GÜLER
 * @copyright Copyright (c) 2020, EMRE GÜLER
 */
...
// ─── Sözleşme Şablonları (DB'ye yazmadan, doğrudan PHP'den) ───

function sutore_get_on_bilgilendirme() {
	return <<<'EOD'
<div>
...
function sutore_get_mesafeli_satis() {
	return <<<'EOD'
<div>
<h3>1. Taraflar</h3>
...
// ─── Ortak: Ürün satır HTML'i oluştur ────────────────────

function sutore_contract_product_row($value) {
	return '<tr>'
		. '<td class="tg-s6z2">' . $value['name'] . '</td>'
		. '<td class="tg-s6z2">' . $value['product_price'] . '</td>'
		. '<td class="tg-s6z2">' . $value['service_cost'] . '</td>'
		. '<td class="tg-s6z2">' . $value['insurance_cost'] . '</td>'
		. '<td class="tg-s6z2">' . $value['total_price'] . '</td>'
		. '</tr>';
}

// ─── Ortak: Fiyat hesaplama ──────────────────────────────

function sutore_contract_calc_prices($price, $quantity, $hizmet_bedeli, $guvence_bedeli) {
	$quantity         = max(1, (int) $quantity); // sıfıra bölme koruması
	$guvence_divisor  = 100 + $guvence_bedeli;
	if ($guvence_divisor == 0) $guvence_divisor = 100; // sıfıra bölme koruması

	$withoutInsurance = ($price - $hizmet_bedeli) * (100 / $guvence_divisor);
	$insuranceFee     = $withoutInsurance * $guvence_bedeli / 100;
	$totalPrice       = $price * $quantity;
	$currency         = get_woocommerce_currency_symbol();

	return [
		'insurance_cost' => $currency . number_format(round($insuranceFee), 2),
		'product_price'  => $currency . number_format($withoutInsurance / floatval($quantity), 2),
		'service_cost'   => $currency . number_format($hizmet_bedeli, 2),
		'total_price'    => $currency . number_format(round($totalPrice), 2),
		'total_raw'      => $totalPrice,
	];
}

// ─── Ortak: Placeholder'ları değiştir ────────────────────

function sutore_contract_replace($template, $data) {
	$search = [
		'[satici-urunler]', '[satici-isim]', '[satici-il]',
		'[kargo-adres]', '[odeme-yontemi]', '[kargo-isim]',
		'[telefon]', '[eposta]', '[toplam-tutar]', '[kargo-tutar]',
	];
	$replace = [
		$data['items']       ?? '',
		$data['fullname']    ?? '',
		$data['city']        ?? '',
		$data['address']     ?? '',
		$data['payment']     ?? 'Kredi Kartı',
		$data['billingName'] ?? '',
		$data['phone']       ?? '',
		$data['email']       ?? '',
		$data['totalPrice']  ?? '',
		$data['shipment']    ?? 'Ücretsiz',
	];
...
if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
	...
	add_action('wp_enqueue_scripts', function() {
		...
		if (is_checkout() && !is_wc_endpoint_url()) {
			wp_enqueue_script('woocontracts_js', plugins_url('/js/woocontracts.js?v=103', __FILE__));
		}
	...
	add_action('admin_enqueue_scripts', function() {
		...
		wp_enqueue_script('woocontractsadmin_js', plugins_url('/js/woocontractsadmin.js', __FILE__));
		wp_enqueue_script('woocontnotify_js', plugins_url('/js/notify.min.js', __FILE__));
	});

	// ─── Checkout: Sözleşmeleri göster ───────────────────

	add_action('woocommerce_after_order_notes', function($checkout) {
		if (!is_checkout()) return;

		$sozlesme_1     = sutore_get_on_bilgilendirme();
		$sozlesme_2     = sutore_get_mesafeli_satis();
		$hizmet_bedeli  = (int) get_option('hizmet_bedeli', 0);
		$guvence_bedeli = (int) get_option('guvence_bedeli', 0);

		$customer = is_user_logged_in() ? wp_get_current_user() : null;
		$merchants = [];

		foreach (WC()->cart->get_cart() as $cart_item) {
			$is_variation = !empty($cart_item['variation_id']);
			$item_id      = $is_variation ? $cart_item['variation_id'] : $cart_item['product_id'];
			$merchant_id  = (int) get_post_field('post_author', $item_id);
			$merchant     = get_user_by('ID', $merchant_id);
			if (!$merchant) continue; // Silinmiş satıcı — atla

			$item_name    = $is_variation ? $cart_item['data']->get_name() : $cart_item['data']->get_title();
			$quantity     = $cart_item['quantity'];
			$price        = $cart_item['data']->get_price();
			$prices       = sutore_contract_calc_prices($price, $quantity, $hizmet_bedeli, $guvence_bedeli);

			if ($is_variation) {
				$merchants[$merchant->ID]['fullname'] = get_user_meta($merchant_id, 'account_name', true) . ' ' . get_user_meta($merchant_id, 'account_lastname', true);
				$merchants[$merchant->ID]['ID']       = $merchant->ID;
			} else {
				$merchants[$merchant->ID]['simple']   = true;
				$merchants[$merchant->ID]['fullname']  = 'sutore Elektronik Hizmetler ve Ticaret Anonim Şirketi';
				$merchants[$merchant->ID]['ID']        = $merchant->ID;
				$merchants[$merchant->ID]['city']      = 'İstanbul';
			}

			$merchants[$merchant->ID]['products'][$item_id] = array_merge(
		...
		foreach ($merchants as $merchant_data) {
			$fullname = $merchant_data['fullname'];

			if (!empty($merchant_data['simple'])) {
				$city = $merchant_data['city'];
			} else {
				$state = get_user_meta($merchant_data['ID'], 'account_city', true);
				$city  = WC()->countries->get_states('TR')[$state] ?? $state;
			}

			$items = '';
			$lastTotal = 0;
			foreach ($merchant_data['products'] as $value) {
				$items .= sutore_contract_product_row($value);
				$lastTotal = $value['total_raw'] ?? 0;
			}
			...
			$data = [
				...
				'city'        => $city,
				'address'     => '',
				'payment'     => 'Ödeme tamamlandıktan sonra sözleşmede belirtilecek',
				'billingName' => '',
				'phone'       => 'Ödeme tamamlandıktan sonra sözleşmede belirtilecek',
				'email'       => $customer ? $customer->user_email : '',
				'totalPrice'  => wc_price($lastTotal),
				'shipment'    => 'Ücretsiz',
			];

			$sozlesme_1_bas .= sutore_contract_replace($sozlesme_1, $data);
			$sozlesme_2_bas .= sutore_contract_replace($sozlesme_2, $data);
		}
		...
		<div id="sozlesmeler">
			<h2>Sözleşmeler</h2>
			<?php echo $sozlesme_1_bas; ?>
			<?php echo $sozlesme_2_bas; ?>
		</div>
		<?php
		$contracts = ob_get_clean();

		$shortcode  = '[lightbox id="wctr-lightbox" width="800px" padding="20px"]';
		$shortcode .= $contracts;
		$text = __('Okudum ve onaylıyorum', 'woocontracts');
		$shortcode .= '<div class="wctr-lightbox__buttons text-center">';
		$shortcode .= sprintf(
			'<button type="button" id="wctr-accept" class="button primary is-underline" aria-label="%s" onClick="document.getElementById(\'woocCheck\').checked = true; jQuery.magnificPopup.close();">%s</button>',
			esc_attr($text), esc_html($text)
		);
		$shortcode .= '</div>';
		$shortcode .= '[/lightbox]';
	...
	add_action('woocommerce_checkout_after_terms_and_conditions', function() {
		$baslik = !empty(get_option('woocontracts_baslik')) ? stripslashes(get_option('woocontracts_baslik')) : 'Sözleşmeler';
		$checkbox_text = sprintf(
			__("I have read sutore's <a class=\"wctrpopuplink\" id=\"contracts\" href=\"#wctr-lightbox\" rel=\"noopener noreferrer\">%s</a> page and accept it", 'sutore'),
			$baslik
		);
		echo '<p class="form-row form-row-last custom-checkboxes">
			<label class="woocommerce-form__label checkbox custom-one">
				<input id="woocCheck" type="checkbox" class="woocommerce-form__input woocommerce-form__input-checkbox input-checkbox" value="1" name="sozlesmeler"><span class="checkmark"></span> <span>' . $checkbox_text . '<span class="required">*</span></span>
			</label>
		</p>';
	});

	// ─── E-posta: Sözleşmeleri ekle ─────────────────────
	...
	add_action('woocommerce_email_customer_details', function($order, $sent_to_admin, $plain_text, $email) {
		...
		$address     = sutore_formatted_billing_address($order);
		...
		foreach ($order->get_items() as $item_id => $order_item) {
			$sold_product = $order_item->get_product();
			if (!$sold_product) continue;

			$is_simple   = $sold_product->is_type('simple');
			$product_id  = $sold_product->get_id();
			$merchant_id = (int) get_post_field('post_author', $product_id);
			$merchant    = get_user_by('ID', $merchant_id);
			if (!$merchant) continue; // Silinmiş satıcı — atla
			...
			if ($is_simple) {
				...
				$merchants[$merchant->ID]['fullname'] = get_post_meta($product_id, 'merchant_name', true);
				$merchants[$merchant->ID]['city']     = WC()->countries->get_states('TR')[get_post_meta($product_id, 'merchant_city', true)] ?? '';
			}

			$merchants[$merchant->ID]['ID'] = $merchant->ID;
			$merchants[$merchant->ID]['products'][$product_id] = array_merge(
				['name' => $order_item->get_name(), 'quantity' => $quantity],
				$prices
			);
		}
		...
		foreach ($merchants as $merchant_data) {
			...
			$city = !empty($merchant_data['simple']) ? $merchant_data['city'] : ($merchant_data['city'] ?? '');
		...
		echo '<h2>Ön Bilgilendirme Formu</h2>';
		echo '<div id="scrollable" style="overflow-y:scroll; height:100px; background: whitesmoke; margin-bottom: 18px; padding: 15px;">';
		echo $sozlesme_1_bas;
		echo '</div>';

		echo '<h2>Mesafeli Satış Sözleşmesi</h2>';
		echo '<div id="scrollable" style="overflow-y:scroll; height:100px; background: whitesmoke; margin-bottom: 18px; padding: 15px;">';
		echo $sozlesme_2_bas;
		echo '</div>';
...
// ─── Yardımcı: Fatura adresi formatla ───────────────────

function sutore_formatted_billing_address($order) {
	$country = $order->get_billing_country();
	$state_code = $order->get_billing_state();
	$states = WC()->countries ? WC()->countries->get_states($country) : [];
	$state  = (!empty($states) && isset($states[$state_code])) ? $states[$state_code] : $state_code;
