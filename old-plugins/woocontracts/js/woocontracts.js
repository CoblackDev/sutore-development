...
// http://bit.ly/magnific-popup#build=inline+image+ajax+iframe+gallery+retina+imagezoom
    ...
    $("input, select, #contracts").on("keyup, change, click", function () {
        $(".billing-name").text($("#billing_first_name").val() + " "+ $("#billing_last_name").val());
        $(".billing-phone").text($("#billing_phone").val());
        $(".billing-email").text($("#billing_email").val());
        $(".shipping-address").text($("#billing_address_1").val() + " "+ $("#billing_address_2").val() + " " + $("#billing_postcode").val() + " " + $("#billing_states option:selected").text() + " " + $("#billing_city option:selected").text() + " " +$("#billing_country option:selected").text());
    });


	if($('.wctrpopuplink').length){
		$('.wctrpopuplink').magnificPopup({
		  type:'inline',
			closeBtnInside:false,
		  midClick: true // Allow opening popup on middle mouse click. Always set it to true if you don't provide alternative source in href.
		});
	}
	if($('a[href="#terms-and-conditions-lightbox"]').length){
		$('a[href="#terms-and-conditions-lightbox"]').magnificPopup({
			type:'inline',
			closeBtnInside:false,
			midClick: true // Allow opening popup on middle mouse click. Always set it to true if you don't provide alternative source in href.
		});
